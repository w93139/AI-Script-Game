...
本文件收纳原先堆在 `readme.md` 顶部的逐轮进度记录。内容原样保留，未作改写——
它们是当时的真实结论，不应该被后来的理解覆盖。

**当前状态请看 [readme.md](readme.md) 顶部的"现在是什么状态"一节。**
本文件只回答"以前发生过什么"，不代表任何一条仍然成立。

新增记录请追加到最上面，并写明日期；不要再往 readme 顶部堆。
