2026-09-20 M4有限推理离线候选：新增私卷1.6/绑定1.8，仅明确选择百炼qwen3.7-flash-2026-07-15的新局SEAL开启固定1024推理；总生成5120、预占5136、正文4096为接纳限。费用分量不重复计、超限结果及回放拒绝、旧局原策略保持。后端3659、预检14与独立复审通过，五选角合成封卷/结算/冷恢复及四份真实授权输入离线容量通过。未启用、未改服务/密钥/前端，真实新增0次/¥0，累计仍175次/¥3.73549424（原200次/¥10）。等待模型路线选择，不将未答复视为批准；真实验证前须完成私有共享账本适配。仍M4，真实语义、桌面麦克风与最终试玩未验收；下一步对应路线的受限固定样本，建议高。详见[有限推理候选](docs/development/M4_BOUNDED_REASONING.md)。

2026-09-20 M4续开发：实际主工程为本目录，资料库ai-jubensha-fusion是较早副本。新增Y两条指代澄清的私有离线候选，真人/AI27题一致；T/Y模拟58调查/82刷新并结算，40旧局保持。后端3524、预检14和独立复核通过，真实新增0次/¥0，累计仍175次/¥3.73549424（原200次/¥10）。未替换3017/API8017。官方现用模型声明思考能力，但推理硬限/计量契约未证实；已询问模型路线，未答复不视为切换批准。下一步M4 A固定失败场景与可验证模型契约；完整MVP未验收，建议高。详见[续开发记录](docs/development/M4_MVP_CONTINUATION.md)。

2026-09-19 M4题面范围候选：新增私卷1.5/绑定1.7，已审题面与授权来源冻结，真人与AI一致；完整选项文字回执校验，旧策略/原题选项/计分保持。后端3524、预检14及独立复核通过；五角色模拟145调查/205刷新可结算，40旧局一致。真实四私卷J/H/Y OK、S UNKNOWN无可用回执；前三份仍有关系或事件依据问题，候选未启用，真实本批未结算。失败局副本可经明确新模拟请求恢复结算，原失败和费用保留。新增4次/¥0.48548200，M4累计175次/¥3.73549424，原200次/¥10保持，剩25次。原7表1397事件、217旧账本、13输入及352本批冻结文件保持；3017/API8017与语音保持，麦克风稍后验。PRD0.38；工程5ab80fc已推送并核验。下一轮仍M4处理具体关系/事件推断，不重复调用未变策略或自动重发S；建议高。 详见[本轮结果](docs/development/M4_SCOPED_FINALE.md)。下文保留历史记录。

2026-09-19 M4来源定位候选：公开1.6、私卷1.4与绑定1.6完成；短物证整卡、实际来源地址和已获地点接入，旧策略/回执保持。后端3418、预检14、独立复核通过，五角色模拟145调查/205刷新可结算。T真实8次：公开3OK/1INVALID、私卷4OK，29调查/41刷新及整局结算通过；114题来源复核完成、四份私卷均仍有实质问题，内容未验收，候选未启用。新增¥0.373942，M4累计171次/¥3.25001224（原200次/¥10），本批8次已封存。原7表1397事件、193旧账本文件和13冻结输入保持；稳定3017/API8017及语音保持。用户真实麦克风稍后测试，不阻塞独立开发。PRD0.37；下一轮M4聚焦题目范围与推断依据，不重复扩大基础流程测试。 详见[本轮结果](docs/development/M4_LOCATED_FINALE.md)。下文保留历史记录。

2026-09-19 M4终局证据候选：公开1.5/table1.3/binding1.5新增逐选项来源、未知及服务端私有核验；旧版本/192输出上限/旧请求保持。后端3313、预检14、独立89项通过；五角模拟145调查/205刷新/结算、40旧视图一致。T真实8次公开2OK/2INVALID、私卷4OK并结算，公开原始内容3/4可支持但实际展示2/4；114题/86选择/51合理未知已审，私卷0/4整卷来源通过，候选未启用。原7表40局/43开场/1397事件、169旧账本、13冻结输入保持。新增8次/¥0.377234，M4累计163次/¥2.87607024，原200次/¥10不变；语音新调用0。运行仍3017/API8017、topic v4/finale1.2/table1.1及前端GOg_ZIZaWQrchzLZGl6h1，用户稍后测麦克风。PRD0.36，工程源码c4a12bb已推送w93139/main并核验，完整阶段记录标签m4-20260919.1。下一步补私卷已获调查地点映射、修引用错位与推断；不能修改已发1.5/1.3提示或回执。仍M4，总验收未过；见[本阶段记录](docs/development/M4_FINALE_EVIDENCE.md)。

2026-09-18 MVP阶段检查点：五角色新版模拟整局145调查/205刷新通过；T真实终局与封卷8OK并结算，但公开来源3/4、私卷4份均未整份有源通过，finale1.4/table1.2候选未启用。后端3224、脱敏定向41、前端114、类型/lint/构建/预检14及独立复核通过。语音单独接回3017/API8017，运行仍topic v4/finale1.2/table1.1，前端GOg_ZIZaWQrchzLZGl6h1；30秒初始化超时释放已实际验证，桌面/390px入口正常，已请用户真实麦克风测试。原7表40局/43开场/1397事件、133旧账本、13冻结输入及3语音回执保持。阶段12次/¥0.292026，M4累计155次/¥2.49883624（原200次/¥10），语音新调用0。PRD0.35。完整源码80文件已同步w93139/AI-Script-Game，6df0b4e及m4-20260918.1远端核验；用户选择真实麦克风稍后测试并继续开发。下一步逐项证据/事件与未知信息约束、固定有源场景，再补五角色真实矩阵，MVP未验收。详见[本阶段证据](docs/development/M4_FINALE_MVP.md)。

## GitHub 阶段同步（2026-09-18 用户追加授权）

- 当前完整前后端源码目标为 `w93139/AI-Script-Game`，本地主分支跟踪 `w93139/main`。每个完成必要检查的开发阶段使用普通提交同步，推送后核对远端 SHA；不再等到整个 MVP 完成才备份。
- 同步前检查远端差异并保留双方改动，不强推。代码候选、已启用版本、真实语义通过与用户验收分别标注，候选备份不冒充正式发布。
- 仅提交程序、合成测试与可公开的开发摘要；凭据、真实存档、商业剧本/图片、原始模型上下文及输出保持仓库外。README 保持产品介绍，阶段进度写入开发文档。
- 本条覆盖下方历史“未经授权不自动 commit/push”及仅后端私有仓库同步的限制；其他安全和数据保护要求继续适用。

2026-09-18 M4来历复验已启用：v4来历任务T公私4题与H真人记账2题共6次真实运行/独立来源全通过，未新增目标金额/缺款推断；旧失败与1.13绑定保持。后端3173、前端62、类型/lint、预检14及独立复核通过；40旧局投影一致，原7表40局/43开场/1397事件、115旧账本和13冻结输入保持，新增18账本。已启动3017/API8017，新对白1.14与v4仅新题使用，前端构建dt1V_zP0Gf7zGDdQ_PpBP保持。首页/原局读取刷新/五选角1470与390px通过，额外调用0。新增6次/¥0.06084000，M4累计143次/¥2.20681024（总200次/¥10）；本轮6次上限封存，未推送。PRD0.33。主线仍M4，等待新局唐小姐失物/来历与恢复试玩；Y/S真人记账、五角新版完整语义、终局推理、当前接回语音/实体手机和正式上线仍未全验。下一轮收试玩反馈并修终局有源推理，建议中等；Key无需重贴。详见[本轮交付与整体进度](docs/development/M4_ORIGIN_ACCEPTANCE.md)。下文为历史检查点。

2026-09-18 M4按题回答任务：新增对白1.14、冻结段落/来源任务和实际片段投影，旧1.13绑定及待定请求保留。固定8题全部真实生成，6条OK、2条INVALID；公开初问为否定范围误伤，已新增仅新题1.4并用原回答完整适配器离线回放通过，原失败保持；来历题有无源的目标金额/动作泛化，v4短任务仅离线验证，仍未启用。真实来源7可支持、1不通过，不冒称8条运行全过。新增8次/¥0.07874600，M4累计137次/¥2.14597024；本轮8次上限已用完且不重置。原7表40局/43开场/1397事件、91旧账本及13冻结输入保持；v3的40旧局投影一致。后端3173、前端62与类型/lint、预检14及独立代码/来源复核通过。3017/8017未启动，前端构建dt1V_zP0Gf7zGDdQ_PpBP不变，未推送；Key鉴权200无需重贴。PRD0.32；下一轮M4先验证v4来历题并有源真实复验，通过后启用/用户试玩，建议中等。详见[本轮交付](docs/development/M4_TOPIC_RESPONSE_TASK.md)。下文为历史检查点。

2026-09-18 M4续测：Key鉴权HTTP200已恢复，无需再粘贴。两轮新增8次/¥0.10014400，2条有源通过、6条INVALID，3项依赖追问未跑；完整提示确实入模。新增仅新题1.3校验，修复“我问谁看见”被误判本人目击，后端3078、前端62、类型/lint、预检14及独立复核通过；五份v3目录仅四个J题切新版本，40旧局投影一致。阿杰漏引/归属与舒望物证因果问题仍未解决，候选未启用，不以离线重放算真实通过。本批含旧UNKNOWN共9次/¥0.17029000，余3次不足完整6例；M4累计129次/¥2.06722424。原7表1397事件、67旧账本、13冻结输入保持。前端构建dt1V_zP0Gf7zGDdQ_PpBP不变；本次3017/8017无监听且未启动服务，未推送。PRD0.31；下一轮仍M4做来源范围离线修复后明确新批次补齐6例，通过再启用/用户验收；建议中等，沿用现有方案。详见[续测记录](docs/development/M4_ANSWER_QUALITY.md)。下文为历史检查点。

# 人生海海 — 项目开发指令

2026-09-18 M4回答质量候选已准备、尚未启用：追加五份私有目录，四份J失物专项引用/时间/动作边界，已消除两类误伤；共享映射加载及新题事件哈希断言、锁内本轮12次/¥0.50保护。40旧局新旧投影一致；后端3066、前端62、类型/lint、预检14与独立复核通过，8条模拟事件/刷新/同通话私聊验证不算真实语义通过。真实新增1次无正文UNKNOWN；只读models两次401 Invalid API key，已请用户更新本机.env第2行，不发聊天。保守新增¥0.070146，M4累计121次/¥1.96708024；原7表1397事件、65旧账本、13原冻结输入保持。当前服务未替换、前端构建dt1V_zP0Gf7zGDdQ_PpBP，PRD0.30，未推送。凭据恢复后先只读鉴权，再固定8例真实有源回归、启用后用户验收；仍M4，建议极高。详见[修复候选](docs/development/M4_ANSWER_QUALITY.md)及[用户验收清单](docs/development/M4_ROLE_ANSWER_ACCEPTANCE.md)。下文为历史检查点。

2026-09-17 M4回答失败恢复已更新本机：原局阿海OK、阿杰因错填引用类别INVALID且正文存在时间/片段覆盖偏差，非Key或网络故障。前端问题/回答前置，失败可明确使用原题资料简答；后续成功规则操作不再重复历史错误。前端62、后端3066、类型/lint/构建、预检14与独立复审通过；原局副本简答→刷新→第二轮阅读、1470/390px通过。原7表40局/43开场/1397事件、65账本文件全保持；本轮真实调用0/费用0。仅更前端，构建dt1V_zP0Gf7zGDdQ_PpBP，PRD0.29。模型生成质量尚未修，不改旧提示/失败/费用，未推送。下一轮仍M4做新题引用/事实版本化修复与有源回归，建议极高；见[失败恢复交付](docs/development/M4_ANSWER_RECOVERY.md)。下文为历史检查点。

2026-09-17 M4试玩交互修复已更新本机：调查区说明通话阻塞并可结束对话恢复调查；顶栏下一阶段定位、剩余点数确认；提问前置并一次确认后用冻结回执请求回复，自由发言明确仅记录，空议题/等待/错误给出说明。前端56、后端3066、预检/自测14、类型/lint/生产构建及独立复审通过；合成实际页面公私回复、调查/推进/刷新及1470/390px通过，真实原局只读复核。40局/43开场/1385事件及7表、59真实账本文件全保持，本轮真实调用/费用0；仅更新前端，构建xOIjudDT1vGjc9891i4qw，PRD0.28。仍M4待用户复测，有限提问不是通用自由聊天，既有模型语义/终局问题未修；下一轮先收操作反馈再做有源语义修复，建议极高。未推送本轮代码；README保持项目介绍。详见[本轮交付](docs/development/M4_INTERACTION_RECOVERY.md)。下文为历史检查点。

2026-09-17 M4真实接回已更新本机：真实五角色目录、358来源、55授权图片及批准49号采用稿已接入；玩家无后补标签，真相/评分/调查扣点不变。蚂蚁ant_digital/qwen3.6-plus v2修复JSON提示、同schema重申、纯text_tokens用量与按冻结绑定的窗口计量；终局1.2补有限归属/矛盾提示，旧版本哈希和事件保持。后端3066、前端43、类型/lint/生产构建、预检/自测14与独立审查通过；五角最终模拟145调查/260刷新/每角55图通过，T真实29调查/4交流/4AI封卷/结算/52刷新通过。语义另计：4交流3有源/1归属歧义；4终局2可支持/2事件混同且全部因双句格式留空；私密封卷仍有推理错误，不算内容全过。原7表、36局/38开场/1234事件及184旧账本文件保持；真实副本39局/42开场/1381事件，离线副本独立。本轮26次/保守¥0.83056200，M4累计118次/¥1.87613824，原¥10/200次上限保持。入口http://127.0.0.1:3017（API8017），构建n16GGDLLwAx-Yxp5bx7p0，PRD0.27；未提交推送/正式发布，语音与实体手机仍待验。下一轮M4修终局格式、归属和身份/同体推理，交付有源对照与受限真实回归；建议极高。详见[本轮交付](docs/development/M4_REAL_RECONNECT.md)。下文为历史检查点。

2026-09-16 49号线索已按用户授权完成采用稿：用户无法提供原扫描，要求依据上下文创作且玩家卡面不显示后补。私有 authored-clue-49-20260916 内有卡面/卡背、来源与接入清单；沿用M4物证强度、card-49-r和管家房同次发放，不改真相/评分/扣点。21处相关问卷文案已准备自然化，内存副本结构核对与独立逻辑审查通过；未接入运行、未改冻结包/旧局。可玩缺口已解决，不再要求用户补49原件；档案仍记原件缺失。详见 [49号采用稿](docs/development/M4_CLUE49_ADOPTION.md)。下一步仍M4真实资料与蚂蚁接回，建议极高。

2026-09-16 资料与API定位补记：已在桌面旧项目 AI人生海海剧本杀/private-data 找回真实资料、五角色目录与旧存档；原清单362/362、修复包来源358/358、展示图55/55实存且SHA匹配。原版49号卡正反面仍缺，只有重建候选。旧路径须映射，未启动/导入存档。用户使用蚂蚁数科DTMaaS，私有.env已保存ANT_MAAS_API_KEY，只读模型列表HTTP200；当前游戏仍未适配蚂蚁，生成调用0，付费开关关闭。详见 [资料与蚂蚁核查](docs/development/M4_ANT_AND_DATA_INTAKE.md)。此前“缺资料/配置”按当时检查范围理解，现为已定位待接回。

2026-09-16 新版前端核心流程已接回：恢复锁定依赖；修复禁止 null target、自动首选角/首地点、真实页面混演示、错误私聊入口及缺失封卷表单。授权资料、回忆、有限议题、有源简答、原键恢复、换号隔离和首页记录已接线。五角色合成 HTTP 整局及一角色浏览器公私交流到结算/刷新通过；真实调用/费用 0。测试及限制见 [M4 新版接回](docs/development/M4_FRONTEND_RECONNECT.md)。当前缺真实 .env/private-data，商业包/角色目录在仓库外；图片呈现、登录续期、真实模型/语音和实体手机待验。主线仍 M4，先找回资料接回真实内容，再验五角色。完整开源仓库已按用户指令在 w93139/AI-Script-Game 创建，本轮本地修改尚未提交推送。下文为历史检查点。

2026-09-11 M4 五角色与终局公示已更新本机：私有 bootstrap 链入 H/Y/S，T/J/H/Y/S 五选角；新终局四 AI 各一句、一次生成/失败留空、全部封卷后实际票复用原句，评分不改。1.1 补已获物证调查地点和有限转述核验，旧绑定/话语/费用保持。后端3022、前端必需八套353、类型/lint/构建/预检16及独立代码复审通过；H/Y/S真实流程、H/S新策略复验及1470/390px DOM/刷新通过。首批12句9可支持/2地点错误/1归属风险，新8句6可支持/2归属或相似关系错误，语义仍未全过；不以结构或结算通过外推推理正确。本轮40次调用/保守¥0.63646320，M4累计92次/¥1.04557624（授权¥10）；原7表含934旧事件均保持，最终36局/38开场/1234事件。构建VyNEFOyFC-aPvHD9opF6j，PRD0.25。用户明确要求完整前后端推送hubooooooo/AI-Script-Game。下一轮M4按试玩反馈修复语义/归属，交付有源对照及回归；建议极高。详见[本轮交付](backend/docs/M4_FIVE_ROLES_FINALE.md)。下文为历史检查点。
...
- 用户已要求后端保存到 GitHub，后续版本迭代同步更新。目标为私有仓库 `hubooooooo/ai-jubensha-backend`；保留现有公开 `origin` 与本地工作区，不将本轮工作直接推入公开仓库。
...
用户于2026-09-18补充：每轮开发结束都要汇报整体开发进度与验收情况。明确当前里程碑、已完成和未完成事项、主要阻塞；分别说明自动检查、真实模型和用户试玩的通过/失败/未测项，是否已启用及需要用户验收的具体操作。不要只报本轮测试数量，也不要用猜测的完成百分比替代整体进度。

用户于2026-09-18确认持续推进到本机稳定试玩MVP：一真人四AI、五种真人选角完整整局、恢复与费用可控，公网部署后续做。按[总计划](docs/development/MVP_DELIVERY_PLAN.md)持续完成已授权工作，不因单个专项待用户试玩而停止独立开发；原M4 200次/¥10及语音独立限制保持，用户验收不能由自动检查替代。
