...
const serviceModule = compile('../src/services/packageSpeechInputService.ts', name => {
  if (name === '@/services/auth') return { currentToken: () => token };
  if (name === '@/lib/config') return { API_BASE_URL: 'https://voice.invalid' };
  return require(name);
});
...
test('speech service sends only audio and current input binding with authorization and no-store', async t => {
  ...
  assert.equal(calls[0].options.body, audio); assert.equal(calls[0].options.signal.aborted, false);
...
function harness(overrides = {}) {
  ...
  const Component = compile('../src/components/play/PlayVoiceInput.tsx', name => {
    ...
    if (name === '@/services/playSpeechAuth') return { watchPackagePlayAuth(callback) { authCallback = callback; return { isCurrent: () => authCurrent, dispose() {} }; } };
    return require(name);
  }).default;
...
test('HTTP timeout preserves the request id for receipt recovery and never re-uploads', async t => {
  let expire, attempts = 0, aborted = 0;
  t.mock.method(global, 'setTimeout', callback => { expire = callback; return 1; });
  ...
  const pending = service.transcribe('play', 'same-request', new Blob(['fixture']), { revision: 4, channel: 'PUBLIC' }, new AbortController().signal);
  expire();
  await assert.rejects(pending, error => error.status === 504);
  assert.equal(attempts, 1); assert.equal(aborted, 1);
});

test('aborting an HTTP request immediately cancels its transport without sending again', async t => {
  ...
  const pending = service.receipt('play', 'same-request', controller.signal);
  controller.abort();
