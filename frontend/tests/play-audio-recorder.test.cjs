...
function harness(config = {}) {
  ...
  class Context {
    ...
    resume() {
      if (config.resume) return config.resume.promise.then(() => { if (this.state !== 'closed') this.state = 'running'; });
...
test('a pending worklet or context resume times out, releases input and cannot start after late completion', async () => {
  for (const dependency of ['module', 'resume']) {
    const pending = deferred(), h = harness({ [dependency]: pending });
    let outcome;
    void h.start().then(value => { outcome = value; }, error => { outcome = error; });
    await settle();
    assert.equal(outcome, undefined); assert.equal(h.track.readyState, 'live');
    h.fire(30000); await settle();
    assert.equal(outcome?.code, 'STARTUP_TIMEOUT');
    assert.match(outcome.message, /启动超时/); released(h);
    assert.equal(h.nodes.length, 0); assert.equal(h.errors.length, 0);
    pending.resolve(); await settle();
    assert.equal(h.nodes.length, 0); assert.deepEqual(h.progress, []);
    assert.equal(h.contexts[0].state, 'closed');
  }
});

test('unanswered permission times out and a late grant is immediately stopped', async () => {
  const permission = deferred(), h = harness({ permission });
  let outcome;
  void h.start().then(value => { outcome = value; }, error => { outcome = error; });
  await settle(); h.fire(30000); await settle();
  assert.equal(outcome?.code, 'STARTUP_TIMEOUT');
  assert.equal(h.contexts[0].closed, 1); assert.equal(h.timers.size, 0);
  permission.resolve(h.stream); await settle();
  released(h); assert.equal(h.track.stops, 1); assert.equal(h.nodes.length, 0);
  assert.equal(h.errors.length, 0); assert.deepEqual(h.progress, []);
});

test('successful initialization clears its timeout without shortening recording', async () => {
  const h = harness(), recording = await h.start();
  assert.ok([...h.timers.values()].every(timer => timer.ms !== 30000));
  h.fire(30000); h.nodes[0].feed(tone(4800)); await settle();
  assert.equal(h.track.readyState, 'live'); assert.equal(h.contexts[0].state, 'running');
  assert.equal((await recording.stop()).size, 9644); released(h);
