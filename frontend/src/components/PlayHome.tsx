...
const action = 'player-action player-action-primary';
const secondary = 'player-action player-action-secondary';
...
function SavedGame({ item, recent = false }: { item: PlayLibraryItem; recent?: boolean }) {
  return <article className={`${card} saved-story ${recent ? 'saved-story-recent' : ''}`}>
    <div className="saved-story-copy">
      <p className="player-eyebrow">{recent ? '最近一局' : item.settled ? '已结束' : '进行中'}</p>
      <h3 className="saved-story-title">{item.title}</h3>
      <p className="mt-3 text-sm text-mist">{item.character_name} <span className="px-1 text-faint">/</span> {item.phase_label}</p>
      <p className="mt-3 text-xs text-faint">最近进度 <time className="tabular-nums" dateTime={item.updated_at}>{new Date(item.updated_at).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</time></p>
    </div>
    <Link className={recent ? action : secondary} href={savedPlayHref(item)}>{item.settled ? '查看结局' : '继续游戏'}<span aria-hidden="true" className="player-action-arrow">→</span></Link>
  </article>;
}
...
export function PlayerLibrary({ recordsOnly = false }: { recordsOnly?: boolean }) {
  ...
  return <>
    <section aria-label="我的游戏记录" className="player-library-section space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="player-section-title">{recordsOnly ? '我的记录' : '回到故事里'}</h2>{!recordsOnly && <Link className={secondary} href="/play/records">查看全部记录</Link>}</div>
      ...
      {recordsOnly ? <div className="grid gap-4 sm:grid-cols-2">{items.map(item => <SavedGame key={item.play_id} item={item} />)}</div> : items[0] && <SavedGame item={items[0]} recent />}
      {recordsOnly && hasMore && <button className={secondary} disabled={loading} onClick={() => void loadPage()}>{loading ? '正在读取……' : '加载更多记录'}</button>}
    </section>
    {!recordsOnly && <section aria-label="开始新游戏" className="player-catalog space-y-4">
      <h2 className="player-section-title">开始新游戏</h2>
      ...
      <div className="player-story-list">{releases.map(item => <article key={item.id} className="player-story-row">
        <div className="min-w-0"><p className="player-eyebrow">单人推理 · AI 角色陪伴</p><h3 className="mt-2 break-words text-2xl font-medium text-paper">{item.title}</h3><p className="mt-2 text-sm text-faint">{item.player_count} 个角色</p></div>
        <Link className={secondary} href={`/play/package-preview?release_id=${item.id}`}>选择角色<span aria-hidden="true" className="player-action-arrow">→</span></Link>
      </article>)}</div>
    </section>}
  </>;
}
...
export default function PlayHome({ recordsOnly = false }: { recordsOnly?: boolean }) {
  ...
  return <AppLayout><div className="player-home">
    <header className="player-home-header"><div><p className="player-eyebrow">人生海海</p><h1 className="player-home-title">{recordsOnly ? '我的游戏记录' : '每个故事，都由你走进去'}</h1><p className="mt-3 text-sm leading-6 text-mist">{recordsOnly ? '继续未完的推理，或重温已经揭晓的故事。' : '独自进入故事，与 AI 角色交流，寻找属于你的答案。'}</p></div>{recordsOnly && <Link className={secondary} href="/">返回首页</Link>}</header>
