...
import { watchPackagePlayAuth } from '@/services/playSpeechAuth';
...
let nextSessionId = 0;
const buttonStyle = 'min-h-11 rounded-lg border border-smoke px-3 py-2 text-sm text-paper disabled:opacity-40';
...
export default function PlayVoiceInput(props: PlayVoiceInputProps) {
  ...
  return <section aria-label={`${providerName ?? ''}语音输入`} className="space-y-2">
    {!visible ? <button ref={opener} type="button" disabled={!enabled} className={buttonStyle} onClick={() => void open()}>语音输入</button>
      : <div className="space-y-3 rounded-lg border border-graphite bg-obsidian p-3">
        ...
        {visible.message && <p role={visible.stage === 'ERROR' ? 'alert' : 'status'} className="text-sm leading-6 text-acid-lime">{visible.message}</p>}
        ...
        {visible.realtime && ['RECORDING', 'TRANSCRIBING'].includes(visible.stage) && <div className="rounded-lg border border-smoke bg-carbon p-3" aria-label="实时识别文字" aria-live="off">
        ...
        {visible.stage === 'REVIEW' && <>
          <label className="block text-sm">识别文字（可修改）<textarea ref={previewInput} value={visible.text} rows={3} maxLength={12000} className="mt-2 w-full rounded border border-smoke bg-carbon p-3 text-paper" onChange={event => { const session = run.current; if (session?.id === visible.sessionId && current(session)) setState(value => value?.sessionId === session.id ? { ...value, text: event.target.value } : value); }} /></label>
          <p className={`text-xs leading-6 ${tooLong ? 'text-acid-lime' : 'text-mist'}`}>填入后 {Array.from(merged.trim()).length} / 1000 字符。{tooLong ? '请缩短识别文字或原草稿后再填入。' : '已有文字会保留，识别文字追加到末尾。'}</p>
