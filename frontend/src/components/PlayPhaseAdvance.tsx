...
export default function PlayPhaseAdvance({ view, disabled, onContinue, hiddenMemorySequences = [], renderVisual }: {
  ...
  return <Dialog.Root open={open} onOpenChange={value => { if (!value) setDialog(null); }}>
    <Dialog.Trigger asChild><button type="button" aria-label="进入下一阶段" disabled={disabled} className="min-h-11 rounded-md bg-brass px-4 py-2 text-sm font-semibold text-ink transition-colors hover:bg-white/90 disabled:opacity-40"
    ...
    <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[75] bg-black/60" />
      <Dialog.Content className={`fixed left-3 right-3 top-1/2 z-[75] flex max-h-[calc(100dvh-24px)] -translate-y-1/2 flex-col rounded-xl border border-line bg-panel p-5 text-paper shadow-xl sm:left-1/2 sm:right-auto sm:w-[min(620px,calc(100vw-24px))] sm:-translate-x-1/2`}>
        <div className="flex shrink-0 items-center justify-between gap-3"><Dialog.Title className="text-lg font-semibold">{dialog?.recap ? `${phase?.title || '本阶段'} · 阶段复盘` : '进入下一阶段前，需要复盘吗？'}</Dialog.Title><Dialog.Close asChild><button type="button" aria-label="关闭阶段复盘" className="min-h-11 shrink-0 rounded px-2 text-mist hover:bg-raised"><X aria-hidden="true" className="h-5 w-5" /></button></Dialog.Close></div>
        ...
        <div className="mt-5 flex shrink-0 flex-wrap gap-3 border-t border-line pt-4">
          {!dialog?.recap && <button type="button" className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-md border border-line px-4 py-2 text-sm text-mist hover:bg-raised" onClick={() => { if (open) setDialog({ revision: view.revision, recap: true }); }}><History aria-hidden="true" className="h-4 w-4" />需要复盘</button>}
          <button type="button" disabled={disabled} onClick={finish} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-md bg-brass px-4 py-2 text-sm font-semibold text-ink disabled:opacity-40">继续推理<ArrowRight aria-hidden="true" className="h-4 w-4" /></button>
        </div>
      </Dialog.Content>
    </Dialog.Portal>
  </Dialog.Root>;
