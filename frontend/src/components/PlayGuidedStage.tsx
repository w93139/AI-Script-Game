...
export default function PlayGuidedStage({ view, locked, onGuided }: { view: PackagePlay; locked: boolean; onGuided?: (action: GuidedAction) => void }) {
  ...
  return <section aria-label="选择调查地点" className="min-w-0 space-y-4 rounded-xl border border-line bg-panel p-5">
    ...
    <p className="text-mist">调查点：{remaining} / {view.mechanics?.initial_points ?? 0}{batch && <span className="ml-3">已选 {selected.length} 处 · 消耗 {cost} 点</span>}</p>
    ...
    <div className="grid gap-3 sm:grid-cols-2">{actions.map(item => <div key={item.id} className={`rounded-md border p-4 ${selected.includes(item.id) ? 'border-line bg-white/5' : 'border-line'}`}>
      <h3 className="break-words font-medium">{item.label}</h3><p className="mt-2 text-sm text-mist">消耗 {item.cost} 点</p>
      {batch ? <label className="mt-2 flex min-h-11 cursor-pointer items-center gap-3 text-sm text-mist"><input type="checkbox" className="h-5 w-5 accent-brass" aria-label={`选择${item.label}`} checked={selected.includes(item.id)}
