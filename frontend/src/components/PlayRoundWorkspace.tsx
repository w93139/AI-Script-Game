...
export default function PlayRoundWorkspace({ view, hiddenMemorySequences = [], renderVisual, onCollect, locked }: {
  ...
  const button = 'min-h-11 rounded-md border border-line px-3 py-2 text-left text-sm text-mist hover:bg-raised';
  const renderIndex = (entries = phases) => <div className="space-y-3">{entries.map(entry => <details key={entry.phase_id} open={entries.length === 1 || entry.phase_id === view.current_phase.id} className="rounded-md border border-line p-3">
    ...
    <details><summary className="min-h-11 cursor-pointer py-2 text-xs text-mist">全部资料与记录</summary>
    ...
    {entry.investigations.map((investigation, i) => <details key={`${investigation.sequence}:${i}`} className="mb-2 rounded border border-line px-2"><summary className="min-h-11 cursor-pointer py-2 text-xs leading-5 text-paper">{investigation.mode === 'LEGACY' ? '旧调查记录' : `${investigation.character_id === view.selected_character_id ? '我' : name(investigation.character_id)}选择`} · {investigation.label}{investigation.mode === 'AUTO' ? '（自动安排）' : ''}</summary>
      <p className="text-xs text-mist">消耗 {investigation.cost} 点</p>{investigation.materials.map(ref => { const material = materials(entry).find(item => item.collection === ref.collection && item.id === ref.id); return material ? <button key={`${ref.collection}:${ref.id}`} type="button" className="block min-h-11 w-full break-words py-2 text-left text-xs text-mist" onClick={event => select({ phase: entry.phase_id, material: `${ref.collection}:${ref.id}` }, event.currentTarget)}>{label(material)}</button> : null; })}
      {investigation.materials.length === 0 && <p className="py-2 text-xs text-mist">此记录未关联新增可读线索。</p>}
    </details>)}
    <div className="space-y-1">{materials(entry).filter(ref => !entry.investigations.some(investigation => investigation.materials.some(item => item.collection === ref.collection && item.id === ref.id))).map(ref => <button key={`${ref.collection}:${ref.id}`} type="button" className="block min-h-11 w-full break-words rounded px-2 py-2 text-left text-sm text-mist hover:bg-raised" onClick={event => select({ phase: entry.phase_id, material: `${ref.collection}:${ref.id}` }, event.currentTarget)}>{label(ref)}</button>)}</div>
    {records(entry).length > 0 && <details className="mt-2"><summary className="min-h-11 cursor-pointer py-2 text-sm text-mist">已保存发言</summary>{records(entry).map(record => <button key={record.id} type="button" className="block min-h-11 w-full break-words rounded px-2 py-2 text-left text-xs text-mist hover:bg-raised" onClick={event => select({ phase: entry.phase_id, record: record.id }, event.currentTarget)}>{name(record.speaker)}{'audience' in record ? ' · 私聊' : ' · 公开'}：{Array.from(record.text).slice(0, 24).join('')}</button>)}</details>}
  ...
  const renderMaterial = (ref: Material) => {
    const item = roundMaterial(view, ref);
    if (!item) return null;
    return <section key={`${ref.collection}:${ref.id}`} className="rounded-md border border-line p-4"><h3 className="mb-3 text-sm font-semibold text-mist">{label(ref)}</h3><PlayText text={item.text} />
  ...
  return <>
    <aside aria-label="每轮线索与记录" data-round-rail data-pinned={Boolean(rail.pinned && rail.phase) || undefined} className="play-round-rail fixed bottom-0 left-0 top-0 z-40 hidden border-r border-line bg-panel"
      ...
      <div className="flex h-full flex-col items-center gap-2 overflow-y-auto px-1 py-3">
        <BookOpen aria-hidden="true" className="mb-1 h-5 w-5 shrink-0 text-mist" /><span className="sr-only">已到达的步骤</span>
        {phases.map((entry, i) => <button key={entry.phase_id} type="button" className="play-round-tab flex min-h-14 w-11 shrink-0 flex-col items-center justify-center rounded-md border border-transparent text-mist hover:bg-raised hover:text-paper focus-visible:outline-2 focus-visible:outline-mist"
      ...
      {rail.phase && <section id={peekId} data-round-peek aria-label="展开的轮次记录" className="play-round-peek absolute bottom-0 top-0 flex min-w-0 flex-col border-r border-line bg-panel p-3 text-paper shadow-xl">
        <div className="mb-3 flex shrink-0 items-center justify-between gap-2"><h2 className="text-sm font-semibold">{phases.find(entry => entry.phase_id === rail.phase)?.title}</h2>
          <div className="flex items-center gap-1"><button type="button" aria-pressed={rail.pinned} aria-label={rail.pinned ? '取消固定轮次记录' : '固定轮次记录'} className="min-h-11 rounded px-2 text-mist hover:bg-raised" onClick={() => setRail(current => ({ ...current, pinned: !current.pinned }))}><Pin aria-hidden="true" className="h-4 w-4" /></button><button type="button" aria-label="收起轮次记录" className="min-h-11 rounded px-2 text-mist hover:bg-raised" onClick={closeRail}><X aria-hidden="true" className="h-4 w-4" /></button></div>
        </div>
    ...
    <Dialog.Root open={open} onOpenChange={value => { setOpen(value); if (!value) setSelection(null); }}>
      ...
      <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[70] bg-black/60" />
        <Dialog.Content onCloseAutoFocus={event => { event.preventDefault(); if (trigger.current?.isConnected) trigger.current.focus({ preventScroll: true }); }} className="fixed bottom-3 left-3 right-3 top-3 z-[70] flex min-w-0 flex-col rounded-xl border border-line bg-panel p-4 text-paper shadow-xl sm:left-auto sm:w-[min(640px,calc(100vw-24px))]">
          ...
          <div className="mt-4 min-h-0 space-y-4 overflow-y-auto overscroll-contain">
            {!phase ? renderIndex() : <>
              ...
              {records(phase).filter(record => selection?.recap || record.id === selection?.record).map(record => <section key={record.id} className="rounded-md border border-line p-4"><h3 className="mb-3 text-sm text-mist">{name(record.speaker)} · {'audience' in record ? '双方私聊说法' : '公开说法'}</h3><PlayText text={record.text} /></section>)}
              {selection?.recap && <section className="space-y-2 rounded-md border border-line p-4"><h3 className="font-semibold">调查记录</h3>{phase.investigations.length ? phase.investigations.map((entry, i) => <p key={`${entry.sequence}:${entry.action_id}:${i}`} className="text-sm leading-6">{entry.label} · {entry.cost} 点 · {entry.mode === 'LEGACY' ? '旧调查记录' : `${entry.character_id === view.selected_character_id ? '我' : name(entry.character_id)}选择${entry.mode === 'AUTO' ? '（自动安排）' : ''}`}</p>) : <p className="text-sm text-mist">本步骤没有调查记录。</p>}</section>}
            </>}
          </div>
        </Dialog.Content>
      </Dialog.Portal>
