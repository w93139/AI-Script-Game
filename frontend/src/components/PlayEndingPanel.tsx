...
export default function PlayEndingPanel({ view }: { view: PackagePlay }) {
  ...
  const endings = (items: typeof mine) => items.map((ending, i) => <div key={i}>
    <h3 className="text-base font-semibold text-mist">{ending.character_ids.map(name).join('、')}的结局</h3>
  ...
  const scores = (id: string) => result?.totals.filter(t => t.character_id === id).map(total => <details key={id}>
    <summary className="min-h-11 cursor-pointer py-2 text-sm text-mist">{name(id)}：{total.total_points ?? '待核对'} / {total.max_points} 分 · 查看分项得分</summary>
  ...
  return <>
    ...
    {(others.length > 0 || otherTotals.length > 0) && <section aria-label="其他角色结局" className={box}>
      <button type="button" aria-expanded={expanded} aria-controls="play-other-endings" onClick={() => setExpandedScope(expanded ? null : scope)} className="min-h-11 rounded-md border border-line px-4 py-3 text-sm font-semibold text-mist hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-mist">{expanded ? '收起其他角色结局' : '查看其他角色结局'}</button>
