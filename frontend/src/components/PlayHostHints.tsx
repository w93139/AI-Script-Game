...
export default function PlayHostHints({ view, locked, error, onGuided }: {
  ...
  const button = 'min-h-11 rounded-md border border-line px-3 py-2 text-sm text-mist hover:bg-raised disabled:opacity-40';
  ...
  return <Dialog.Root open={open} onOpenChange={value => { setOpen(value); if (!value) setConfirm(null); }}>
    <Dialog.Trigger asChild><button type="button" className="inline-flex min-h-11 items-center gap-2 rounded-md border border-line bg-white/10 px-3 py-2 text-sm font-semibold text-mist hover:bg-white/20"><CircleHelp aria-hidden="true" className="h-5 w-5 shrink-0" />{view.settled ? '结局答疑' : '询问主持人'}</button></Dialog.Trigger>
      ...
      <Dialog.Content className="fixed bottom-4 left-3 right-3 z-[70] flex max-h-[85dvh] min-w-0 flex-col rounded-xl border border-line bg-panel p-4 text-paper shadow-xl sm:left-auto sm:right-6 sm:w-[480px]">
        ...
        <div className="mt-4 min-h-0 overflow-y-auto overscroll-contain">
          {view.settled ? <div className="space-y-4">
            ...
            {qaAnswer ? <section aria-label="主持人解答" className="rounded-md border border-line bg-ink p-4"><h3 className="mb-3 font-medium text-mist">{qaAnswer.title}</h3><PlayText text={playerHostText(qaAnswer.text)} /></section> : <p className="text-sm text-mist">{qaQuestions.length ? '选一个问题，听主持人解答。' : '可以先回看结尾与真相复盘。'}</p>}
          </div> : <>
          ...
          {topic && <><h3 className="mt-4 font-medium">{topic.title}</h3>
            ...
            {entries.map(entry => <section key={`${entry.topic_id}:${entry.level}`} className="mt-3 rounded-md border border-line p-3"><h4 className="mb-2 text-sm text-mist">{entry.level === 1 ? '一点方向' : entry.level === 2 ? '关键提示' : '问题答案'}</h4><PlayText text={playerHostText(entry.text)} /></section>)}
          ...
          {history.length > 0 && <details className="mt-5 rounded-md border border-line p-3"><summary className="min-h-11 cursor-pointer py-2 text-sm text-mist">回看已保存的提示（{history.length} 条）</summary><div className="mt-3 space-y-3">{history.map(entry => <section key={`${entry.topic_id}:${entry.level}`} className="rounded-md border border-line p-3"><h3 className="mb-2 text-sm font-medium">{entry.title} · {entry.level === 1 ? '一点方向' : entry.level === 2 ? '关键提示' : '问题答案'}</h3><PlayText text={playerHostText(entry.text)} /></section>)}</div></details>}
        ...
        <AlertDialog.Root open={confirm !== null} onOpenChange={value => { if (!value) setConfirm(null); }}><AlertDialog.Portal>
          ...
          <AlertDialog.Content className="fixed left-3 right-3 top-1/2 z-[80] -translate-y-1/2 rounded-xl border border-line bg-panel p-5 text-paper shadow-xl sm:left-1/2 sm:w-[420px] sm:-translate-x-1/2">
