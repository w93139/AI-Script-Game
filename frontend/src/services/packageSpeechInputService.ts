...
export async function request(path: string, options: RequestInit): Promise<unknown> {
  ...
  const timer = setTimeout(cancel, options.body ? 65_000 : 15_000);
  options.signal?.addEventListener('abort', cancel, { once: true });
  if (options.signal?.aborted) cancel();
  try {
    const response = await fetch(`${API_BASE_URL}/api/fusion${path}`, {
      ...options, signal: controller.signal, cache: 'no-store', headers: { Authorization: `Bearer ${token}`,
        ...(options.body ? { 'Content-Type': 'audio/wav' } : {}) },
    });
    if (currentToken() !== token) throw new SpeechInputError(401, messageFor(401));
    ...
    if (controller.signal.aborted && !options.signal?.aborted) throw new SpeechInputError(504, messageFor(504));
    throw cause;
  } finally {
    clearTimeout(timer);
