...
export type PlayAudioRecordingErrorCode = 'UNSUPPORTED' | 'INSECURE_CONTEXT' | 'PERMISSION_DENIED'
  | 'DEVICE_UNAVAILABLE' | 'CANCELLED' | 'STARTUP_TIMEOUT' | 'RECORDING_FAILED' | 'TOO_SHORT' | 'SILENT';
...
const messages: Record<PlayAudioRecordingErrorCode, string> = {
  ...
  CANCELLED: '录音已取消。',
  STARTUP_TIMEOUT: '麦克风启动超时，请检查浏览器权限后重新录音，或继续键盘输入。',
...
export async function startPlayAudioRecording(options: PlayAudioRecordingOptions): Promise<PlayAudioRecording> {
  ...
  const cleanup = () => {
    releaseInput(); clearTimeout(startupTimer); clearTimeout(flushTimer);
  ...
  try {
    // Permission, context resume and worklet loading may never settle. Bound the whole
    // startup and release acquired resources; a late permission grant is disposed below.
    startupTimer = setTimeout(() => fail(new PlayAudioRecordingError('STARTUP_TIMEOUT'), false), STARTUP_TIMEOUT_MS);
