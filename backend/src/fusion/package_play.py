...
from src.fusion.package_table_model import (PackageTableModel, BoundPackageTableModel, ReasonedPackageTableModel, EvidencePackageTableModel, LocatedPackageTableModel, ScopedPackageTableModel, BoundedScopedPackageTableModel,
    TABLE_MODEL_CONTRACTS, REASONED_MODEL_CONTRACT, EVIDENCE_MODEL_CONTRACT, LOCATED_MODEL_CONTRACT, SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT, validate_evidence_decision, table_metadata, validate_table_decision, table_context_window)
from src.fusion.finale_reasoning import reasoning_policy, require_reasoning_base, require_reasoning_budget
...
BOUNDED_TABLE_BINDING_CONTRACT = 'package-text-play-binding/1.8'
...
class PackagePlayService(FinaleMotivationMixin):
    def __init__(self, db: Session, publisher: PublicationReader | None = None,
        ...
        table_types = {'package-table-model/1.0': PackageTableModel,
                       ...
                       BOUNDED_MODEL_CONTRACT: BoundedScopedPackageTableModel}
        ...
        self.legacy_table_policy = legacy_table_policy if legacy_table_policy is not None else (
            self.table_model.model_contract if self.table_model.model_contract not in (REASONED_MODEL_CONTRACT, EVIDENCE_MODEL_CONTRACT, LOCATED_MODEL_CONTRACT, SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT) else 'package-table-model/1.0')
    ...
    def _resolve(self, row: ScriptPackagePlay) -> tuple[dict, dict]:
        try:
            ...
            if package.get('schema_version') == 'script-package/1.4':
                ...
                if binding['schema_version'] in (FULL_BINDING_CONTRACT, FINALE_BINDING_CONTRACT, TABLE_BINDING_CONTRACT, EVIDENCE_TABLE_BINDING_CONTRACT, LOCATED_TABLE_BINDING_CONTRACT, SCOPED_TABLE_BINDING_CONTRACT, BOUNDED_TABLE_BINDING_CONTRACT):
                    ...
                    if binding['schema_version'] == FINALE_BINDING_CONTRACT:
                    ...
                    elif binding['schema_version'] in (TABLE_BINDING_CONTRACT, EVIDENCE_TABLE_BINDING_CONTRACT, LOCATED_TABLE_BINDING_CONTRACT, SCOPED_TABLE_BINDING_CONTRACT, BOUNDED_TABLE_BINDING_CONTRACT):
                        expected_policy = {TABLE_BINDING_CONTRACT: REASONED_MODEL_CONTRACT,
                                           ...
                                           SCOPED_TABLE_BINDING_CONTRACT: SCOPED_MODEL_CONTRACT,
                                           BOUNDED_TABLE_BINDING_CONTRACT: BOUNDED_MODEL_CONTRACT}[binding['schema_version']]
                        ...
                        if binding['schema_version'] in (SCOPED_TABLE_BINDING_CONTRACT, BOUNDED_TABLE_BINDING_CONTRACT):
                        ...
                        if binding['schema_version'] == BOUNDED_TABLE_BINDING_CONTRACT:
                            require_reasoning_base(binding['model'])
                            require_reasoning_budget(binding['budget'])
                            frozen = reasoning_policy(output['max_output_tokens'])
                            if binding['seal_reasoning_policy'] != frozen:
                                raise ValueError('FINALE_REASONING_POLICY_MISMATCH')
                            expected['seal_reasoning_policy'] = frozen
    ...
    def _bound_question_scopes(binding, actor):
        if binding.get('schema_version') not in (SCOPED_TABLE_BINDING_CONTRACT, BOUNDED_TABLE_BINDING_CONTRACT):
            return []
    ...
    def create(self, body, owner: int) -> dict:
        ...
        try:
            ...
            if isinstance(state.engine, PackageFullPlayRules):
                ...
                if self.table_model.model_contract in (REASONED_MODEL_CONTRACT, EVIDENCE_MODEL_CONTRACT, LOCATED_MODEL_CONTRACT, SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT):
                    binding.update(schema_version={REASONED_MODEL_CONTRACT: TABLE_BINDING_CONTRACT,
                                                  ...
                                                  SCOPED_MODEL_CONTRACT: SCOPED_TABLE_BINDING_CONTRACT,
                                                  BOUNDED_MODEL_CONTRACT: BOUNDED_TABLE_BINDING_CONTRACT}[self.table_model.model_contract],
                                   table_policy=self.table_model.model_contract,
                                   finale_motivation_policy=self.finale_policy)
                    if self.table_model.model_contract in (SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT):
                        binding['finale_question_scopes'] = self._freeze_question_scopes(package)
                    if self.table_model.model_contract == BOUNDED_MODEL_CONTRACT:
                        try:
                            require_reasoning_base(binding['model'])
                            require_reasoning_budget(binding['budget'])
                        except ValueError as exc:
                            raise PackagePlayError(str(exc)) from None
                        binding['seal_reasoning_policy'] = reasoning_policy(self.full_output['max_output_tokens'])
    ...
    def _consume(self, state: Replay, kind: str, request: dict, data: dict, binding: dict) -> None:
        ...
        if kind == "AI_REQUEST":
            ...
            if deciding and not calling and (data.get('model') != table_metadata(self._full_table_base(binding), data.get('model', {}).get('schema_version'))
                             ...
                             or (binding.get('table_policy') is None and data['model']['schema_version'] in (REASONED_MODEL_CONTRACT, EVIDENCE_MODEL_CONTRACT, LOCATED_MODEL_CONTRACT, SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT))):
                raise ValueError
        ...
        if pending is None or set(data) != ({"assessment"} if self._requires_table_evidence(pending) else set()) | {"status", "refs", "usage", "accounted", "received_at"} | ({'decision'} if deciding else {"proposal"} if proposing else {"speech"} if responding else set()):
            raise ValueError
        status = data["status"]
        ...
        if status == "OK":
            ...
            if calling:
                ...
                self._require_bounded_table_usage(binding, pending, data['usage'])
                ...
                if self._requires_table_evidence(pending):
                    validate_evidence_decision(decision, data['assessment'], context)
        ...
        if responding and status != "OK" and data["speech"] is not None:
            raise ValueError
        if self._requires_table_evidence(pending) and status != 'OK' and data['assessment'] is not None:
            raise ValueError
    ...
    def _full_table_base(binding):
        ...
        if binding.get('schema_version') == BOUNDED_TABLE_BINDING_CONTRACT:
            base['reserved_output_tokens'] = binding['seal_reasoning_policy']['reserved_output_tokens']
        return base
    ...
    def _table_context(state, binding, character, action, revision=None, version=None):
        ...
        if version in (LOCATED_MODEL_CONTRACT, SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT) and action == 'SEAL_FINALE':
            ...
            if version in (SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT):
                if binding['schema_version'] not in (SCOPED_TABLE_BINDING_CONTRACT, BOUNDED_TABLE_BINDING_CONTRACT):
    ...
    def _table_model_for_policy(self, policy):
        ...
        model_type = {'package-table-model/1.0': PackageTableModel,
                      ...
                      REASONED_MODEL_CONTRACT: ReasonedPackageTableModel,
                       ...
                       LOCATED_MODEL_CONTRACT: LocatedPackageTableModel,
                       SCOPED_MODEL_CONTRACT: ScopedPackageTableModel,
                       BOUNDED_MODEL_CONTRACT: BoundedScopedPackageTableModel}.get(policy)
    ...
    def _prepared_table_adapter(self, prepared):
        ...
        context = prepared.get('source_context') or json.loads(prepared['messages'][1]['content'])['context']
    ...
    def _view(self, row, package: dict, binding: dict, state: Replay) -> dict:
        ...
        if (binding.get('schema_version') in (SCOPED_TABLE_BINDING_CONTRACT, BOUNDED_TABLE_BINDING_CONTRACT)
    ...
    def _requires_table_evidence(pending):
        return bool(pending and pending.get('operation') == 'DECIDE'
                    ...
                    and pending.get('model', {}).get('schema_version') in (EVIDENCE_MODEL_CONTRACT, LOCATED_MODEL_CONTRACT, SCOPED_MODEL_CONTRACT, BOUNDED_MODEL_CONTRACT))

    @staticmethod
    def _require_bounded_table_usage(binding, pending, usage):
        """Recheck normalized limits both before acceptance and on event replay."""
        if binding.get('schema_version') != BOUNDED_TABLE_BINDING_CONTRACT:
            return
        policy = binding['seal_reasoning_policy']
        if (not usage_metadata_is_valid(usage) or set(usage) != set(UsageAmount().to_metadata())
                or not 0 < usage['prompt_tokens'] <= pending['reservation']['prompt_tokens']
                or not 0 < usage['completion_tokens'] <= pending['reservation']['completion_tokens']):
            raise ValueError('FINALE_REASONING_RECEIPT_INVALID')
        reasoning = usage['reasoning_tokens']
        if pending['action'] == 'SEAL_FINALE':
            if (reasoning > policy['reasoning_limit']
                    or usage['completion_tokens'] - reasoning > policy['answer_acceptance_limit']):
                raise ValueError('FINALE_REASONING_RECEIPT_INVALID')
        elif reasoning or usage['completion_tokens'] > binding['full_output']['max_output_tokens'] + policy['reservation_tolerance']:
            raise ValueError('FINALE_REASONING_RECEIPT_INVALID')
    ...
    def _record_result(self, row, binding, state, request_id, status, refs, usage, proposal=None, speech=None, decision=None, motivation=None, assessment=None):
        ...
        if self._requires_table_evidence(state.pending[request_id]):
    ...
    def _finish(self, identifier: str, request_id: str, owner: int, result, expired=False) -> dict:
        ...
        try:
            ...
            try:
                ...
                usage = None
            status, refs, proposal, speech, decision, motivation, assessment = "UNKNOWN", [], None, None, None, None, None
            if expired:
                ...
                if (not current or reason is not None or state.revision != pending["revision"]
                    ...
                    try:
                        if motivating:
                            ...
                            self._require_bounded_table_usage(binding, pending, usage)
                            context = self._table_context(state, binding, pending['character_id'], pending['action'], pending['revision'] - 1)
                            candidate = validate_table_decision(result.get('decision'), context)
                            candidate_assessment = validate_evidence_decision(candidate, result.get('assessment'), context) if self._requires_table_evidence(pending) else None
                            deepcopy(state.engine).table_apply(pending['action'], pending['character_id'], candidate, state.revision + 1)
                            decision, assessment, status = candidate, candidate_assessment, 'OK'
                        elif responding:
                            context = self._dialogue_context(state, binding, pending["character_id"], pending["reply_to"], pending["revision"] - 1, private=private)
                        ...
                        status, speech, proposal, decision, motivation, assessment = "INVALID", None, None, None, None, None
            elif usage is not None:
                status = "INVALID"
            self._record_result(row, binding, state, request_id, status, refs, usage, proposal, speech, decision, motivation, assessment)
